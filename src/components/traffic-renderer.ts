...
import { Point, clamp, quadraticBezier, linearInterpolation, distance, bezierLength } from './math-utils';
import { PfColors } from './pf-colors';
import {
  TrafficPointCircleRenderer,
  TrafficPointConcentricDiamondRenderer,
  TrafficPointRenderer,
  Diamond
} from './traffic-point-renderer';
...
export const decoratedEdgeData = (ele: Cy.EdgeSingular): any => {
  return ele.data();
};
...
export default class TrafficRenderer {
  ...
  private getTrafficEdgeType(edge: any) {
    switch (decoratedEdgeData(edge).traffic.protocol) {
      case Protocol.GRPC:
      case Protocol.HTTP:
  ...
  private fillTrafficEdge(edge: any, trafficEdge: TrafficEdge) {
    ...
    if (trafficEdge.getType() === TrafficEdgeType.RPS) {
      const isHttp = edgeData?.traffic.protocol === Protocol.HTTP;
      const rate = isHttp ? edgeData?.traffic?.rates?.http : edgeData?.traffic?.rates?.grpc;
      const pErr = isHttp ? edgeData?.traffic?.rates?.httpPercentErr : edgeData?.traffic?.rates?.grpcPercentErr;
